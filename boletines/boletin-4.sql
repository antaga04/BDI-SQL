...
WHERE EXISTS (SELECT * FROM TDEPTO D, TCENTR C
                WHERE D.NUMDE = 122 AND D.NUMCE = C.NUMCE AND C.SEÑAS LIKE '%ALCALA%') 
    AND NUMDE = 122 AND SALAR <= (SELECT AVG(SALAR) FROM TEMPLE WHERE NUMDE = 122)
ORDER BY 1;

/* 10 */
SELECT NOMDE, NOMCE, MAX(SALAR) AS MAX
FROM TEMPLE E, TDEPTO D, TCENTR C
WHERE D.PRESU < 60 AND D.NUMDE = E.NUMDE AND C.NUMCE=D.NUMCE
GROUP BY NOMDE, NOMCE HAVING MAX(SALAR) > 2000
ORDER BY 1;